...
You may need to do =M-x all-the-icons-install-fonts=.
...
Note from CCLS's wiki: "When M-x lsp is invoked, projectile is
consulted to locate the project root which determines the associated
lsp-mode workspace. If your project has subprojects,
(projectile-project-root) may think files in the subproject belong to
the child workspace, which is not desired. touch .ccls-root in the
root directory to override projectile roots."

In practice, this means that all C/C++ projects should have
=.ccls-root= at their root directory.
