...
LSP mode will automatically configure =company-lsp= and =lsp-ui=.
...
;; Allow Company to get info from LSP
...
**** C/C++
You must have =ccls= installed (AUR: =ccls= or =ccls-git=).

=ccls= works best with a =compile_commands.json= file at the project
root. Alternatively, a simple =.ccls= can suffice, with
one flag per line; see: [[https://github.com/MaskRay/ccls/wiki/Getting-started][the ccls wiki]]

#+begin_src emacs-lisp

(use-package ccls  :hook ((c-mode c++-mode objc-mode) . (lambda () (require 'ccls) (lsp)))
